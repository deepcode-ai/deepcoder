"""
Functions for reading and writing the `deepcoder.toml` configuration file.
...
default_config_filename = "deepcoder.toml"
...
[deepcoder-app]  # this namespace is used for deepcoder.app, may be used for internal experiments
project_id = "..."

# we support multiple OpenAPI schemas, used as context for the LLM
openapi = [
    { url = "https://api.deepcoder.app/openapi.json" },
...
@dataclass
class Config:
    """Configuration for the Deep Coder CLI and deepcoder.app via `deepcoder.toml`."""

    paths: _PathsConfig = field(default_factory=_PathsConfig)
    run: _RunConfig = field(default_factory=_RunConfig)
    deepcoder_app: _DeepCoderAppConfig | None = None
    ...
    def from_dict(cls, config_dict: dict):
        ...
        # load optional deepcoder-app section
        deepcoder_app_dict = config_dict.get("deepcoder-app", {})
        deepcoder_app = None
        if deepcoder_app_dict:
            assert (
                "project_id" in deepcoder_app_dict
            ), "project_id is required in deepcoder-app section"
            deepcoder_app = _DeepCoderAppConfig(
                # required if deepcoder-app section is present
                project_id=deepcoder_app_dict["project_id"],
                openapi=[
                    _OpenApiConfig(**openapi)
                    for openapi in deepcoder_app_dict.get("openapi", [])
                ]
                or None,
            )

        return cls(paths=paths, run=run, deepcoder_app=deepcoder_app)

    def to_dict(self) -> dict:
        d = asdict(self)
        d["deepcoder-app"] = d.pop("deepcoder_app", None)
