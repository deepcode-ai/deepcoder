"""
The `learning` module is designed to facilitate the collection and storage of user feedback on the outputs generated by the Deep Coder tool. It provides mechanisms for obtaining user consent, capturing user reviews, and storing this information for future analysis and enhancement of the tool's performance.
...
Learning : dataclass
    Encapsulates the metadata and feedback collected during a session of using the Deep Coder tool, including the prompt, model, temperature, configuration, logs, session identifier, user review, and timestamp.
...
ask_collection_consent() -> bool
    Prompts the user for consent to store their data for the purpose of improving Deep Coder.
...
class Learning:
    """
    A dataclass that encapsulates the learning data collected during a Deep Coder session.

    Attributes
    ----------
    prompt : str
        A JSON string representing the prompt provided to Deep Coder.
    model : str
...
def ask_collection_consent() -> bool:
    """
    Asks the user for their consent to store their data for the purpose of improving the Deep Coder tool.
    ...
    """
    answer = input(
        "Is it ok if we store your prompts to help improve Deep Coder? (y/n)"
    )
    ...
    if answer.lower() == "y":
        ...
        print(
            colored(
                "No worries! Deep Coder will not collect your prompts. ❤️",
                "light_green",
            )
        )
        return False
...
def extract_learning(
    ...
    ----------
    prompt : str
        The initial prompt provided to the Deep Coder.
    model : str
        The name of the model used during the session.
    temperature : float
        The temperature setting used for the model's responses.
...
def get_session() -> str:
    ...
    str
        A unique identifier for the user session.
    """
    path = Path(tempfile.gettempdir()) / "deepcoder_user_id.txt"
