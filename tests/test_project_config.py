...
from deepcoder.core.project_config import (
    Config,
    _DeepCoderAppConfig,
    _OpenApiConfig,
    example_config,
...
def test_config_from_dict():
    d = {"deepcoder-app": {"project_id": "..."}}  # minimal example
...
""".strip()
    with tempfile.NamedTemporaryFile(mode="w", delete=False) as f:
        f.write(example_config)
    config = Config.from_toml(f.name)
    config.deepcoder_app = _DeepCoderAppConfig(
        project_id="...",
        openapi=[_OpenApiConfig(url="https://api.deepcoder.app/openapi.json")],
    )
    config.to_toml(f.name)
    assert Config.from_toml(f.name) == config
