...
# Defines the targets help, install, run, and cloc as phony targets.
.PHONY: help install run cloc
...
help:
	...
	@echo "  run <folder_name>  Runs Deep Coder on the folder with the given name."
	@echo "  cloc           	Counts the lines of code in the project."
...
# Defines a target named run. This target will run Deep Coder on the folder with the given name.
run:
	@echo -e "$(COLOR_CYAN)Running Deep Coder on $(COLOR_GREEN)$(name)$(COLOR_CYAN) folder...$(COLOR_RESET)" && \
	poetry run deepcoder projects/$(name)

# Defines a target named cloc. This target will count the lines of code in the project.
cloc:
	@echo -e "$(COLOR_CYAN)Counting lines of code...$(COLOR_RESET)" && \
